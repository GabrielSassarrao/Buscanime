...
export default function InitialResults() {
  ...
  const { theme, allowNsfw } = useTheme();
  ...
  const fetchAnimes = async (pageNumber) => {
    ...
    try {
      ...
      const letterParam = params.letter ? `&letter=${params.letter}` : '';
      const url = `https://api.jikan.moe/v4/anime?order_by=title&sort=asc&limit=24&page=${pageNumber}${letterParam}${nsfwParam}`;
      
      const response = await fetch(url);
      
      if (response.status === 429) {
        if(pageNumber === 1) Alert.alert("Ops", "Muitos acessos. Tente de novo.");
        return;
      }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Stack.Screen 
        options={{ 
          title: `Iniciados com "${params.letter}"`, 
          headerShown: true, 
